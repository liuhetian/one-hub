...
export default function BillingTagStats() {
    ...
    const handleSearch = async () => {
        setSearching(true);
        try {
            const res = await API.get('/api/analytics/billing_tag_stats', {
                params: {
            ...
            if (success) {
                ...
                if (data.length === 0) {
                    ...
                    showSuccess(`查询成功,找到 ${data.length} 个计费分组的数据`);
                }
            } else {
                showError(message);
            }
    ...
    const handleExportCSV = async () => {
        setSearching(true);
        try {
            const res = await API.get('/api/analytics/billing_tag_stats/export', {
                params: {
                    start_time: startDate.format('YYYY-MM-DD'),
                    end_time: endDate.format('YYYY-MM-DD')
                },
            ...
            link.href = url;
            link.setAttribute('download', `billing_tag_stats_${startDate.format('YYYY-MM-DD')}_${endDate.format('YYYY-MM-DD')}.csv`);
    ...
    const quotaChartData = {
        options: {
            ...
            xaxis: {
                categories: statistics.map((s) => s.billing_tag),
    ...
    const modelUsageChartData = {
        options: {
            ...
            labels: modelUsage.map((m) => `${m.billing_tag} - ${m.model_name}`),
        ...
        <>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
                <Stack direction="column" spacing={1}>
                    <Typography variant="h2">计费分组统计</Typography>
                    <Typography variant="subtitle1" color="text.secondary">
                        Billing Tag Statistics
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                        按计费标签分组统计所有令牌的使用情况（优先使用令牌的billing_tag，若为空则使用用户组）
                    </Typography>
                </Stack>
            </Stack>
                                ...
                                >
                                    <Stack spacing={2}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            ...
                                            <Icon icon="solar:tag-bold-duotone" width={32} color={theme.palette.primary.main} />
                                        </Box>
